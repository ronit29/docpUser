                    ...
                    <div className='pkgCardsList d-inline-flex sub-wd-cards top_ipd_list'>
                    	{
                    		this.props.top_data.map((data, i) => {
                    			return <a href={data.url?`${data.url}`:`ipdInfo?ipd_id=${data.id}`} className="pkgcustCards" key={this.props.mergeState?i:data.icon?data.icon:i} onClick={this.navigateTo.bind(this, data)}>
